...
# reads both: average-depth lookups (originally ISClipped._av_depth, which needs a live BAM
...
def write_files(report_table, sum_by_region, is_coords, ref_len, data_folder, cutoff,
                 av_depth, gff_ann_pos) -> None:
    ...
    with open(os.path.join(data_folder, 'karyotype.txt'), 'w') as karyotype:
        ...
        for contig in ref_len.keys():
            karyotype.write('chr - ' + contig + ' ' + contig + ' 0 ' + str(ref_len[contig]) + ' ' +
    ...
    with open(os.path.join(data_folder, 'text.txt'), 'w') as text:
        ...
        for is_name in is_coords.keys():
            text.write(is_coords[is_name][0] + ' ' + is_coords[is_name][1] + ' ' +
                       is_coords[is_name][1] + ' ' + is_name +
                       ' color=vvd' + _cirocs_colors[col_ind % len(_cirocs_colors)] + '\n')
    ...
    with open(data_folder + 'histogram.txt', 'w') as histogram:
        for i in range(len(sum_by_region)):
            ...
            depth = av_depth(sum_by_region.iloc[i]['chrom'],
            ...
            for h in h_columns_is:
                if depth > 0:
                    if sum_by_region.iloc[i][h] / depth / 2 >= cutoff:
                        histogram.write(' '.join(sum_by_region.iloc[i][h_columns].apply(str)) + ' ' +
                                        ','.join(sum_by_region.iloc[i][h_columns_is].apply(
    ...
    with open(data_folder + 'depth.txt', 'w') as depth_hist:
        for contig in gff_ann_pos:
            for ann_id, ann in gff_ann_pos[contig].items():
                if ann[3] - ann[2] <= 0:
                    continue
