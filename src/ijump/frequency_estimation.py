...
def _read_count_mtx(pairs_df, orientation):
    ...
    is_names = dict(
        zip(
            pos_df["IS_name"].unique().tolist(), [i for i in range(pos_df["IS_name"].unique().size)]
        )
    )

    counts = np.zeros((len(pos_df[["Position", "Chrom"]].drop_duplicates()), len(is_names)))
...
def estimate_frequencies(
    ...
    original_rc_l_df = (
        clipped_reads_bwrd.query('clip_position == "left"')
    ...
    original_rc_r_df = (
        clipped_reads_bwrd.query('clip_position == "right"')
        .groupby(["junction_in_read", "IS_chrom"], as_index=False)["Read name"]
        .count()
        .rename(columns={"Read name": "Count", "IS_chrom": "Chrom"})
    )

    original_rc_r: Dict[str, Dict] = {chrom: {} for chrom in pairs_df["Chrom"].unique()}
    ...
    pairs_df["N_overlap_r"] = pairs_df[["Position_r", "Chrom"]].apply(
