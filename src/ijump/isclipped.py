# ISClipped drives both the average and precise detection pipelines.
...
# htslib's own default pileup flag filter. SUPPLEMENTARY(0x800) is
# deliberately kept: iJump is a transposon-insertion caller and supplementary
...
# of silently falling through a dead branch.
...
# An absent side is not evidence, so it is skipped rather than compared: since
# regions can begin at 0 (`max(x - 5, 0)`), an absent side spelled 0 would
# otherwise fall inside one and keep the pair on evidence that was not there.
# NO_JUNCTION being negative would fall outside every region anyway; this
...
class ISClipped:
    def __init__(
        ...
        # In case of preliminary exit an empty table will be generated here.
        ...
        # Region-centric representation: for each region, number of
        # supporting reads for each IS.
        self.sum_by_region = pd.DataFrame()
        self.report_table = pd.DataFrame()
        # Used in precise mode to collect insertion coordinates in the reference.
    ...
    def set_is_boundaries(self, radius):
        ...
        for is_name in self.is_coords.keys():
            ...
            # ASSUMPTION OF COMPLETENESS: if the search area goes outside contig
            # boundaries, every contig is treated as circular.
            if start - radius < 0:
    ...
    def _measure_average_depth(self, chrom, start, stop):
        # Mean coverage over *covered* positions in [start, stop): a position
        # with no read at all does not count toward the denominator, only
        # toward positions some read actually covers.
        #
        ...
        # summing per-read span overlap with the window gives exact coverage
        # without walking CIGAR ops or a per-column pileup.
        window = stop - start
