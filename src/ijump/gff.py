...
class gff:
    ...
    def readgff(self):
        ...
        for c in contig_prop_strings:
            c_name = re.sub(
                r"[^\|]+\|", "", c[0]
            )  # contig name, remove head additional characters separated by "|"
            gff_ges = re.findall(
                "^" + re.escape(c_name) + "\\s[^\n]+", gff_content, re.MULTILINE
            )  # take all annotations from the current chromosome
            x = [" ".join(c)]
            x.extend(gff_ges)
            gff_content_sorted.append("\n".join(x))

        for contig in gff_content_sorted:
            region = re.search(r"\|?([^\s\|]+)\s+\d+\s+(\d+)", contig)
                ...
                if j > 1 and prev_pos < int(fields[3]):
                    ...
                    for k in range(prev_pos + 1, int(fields[3])):
                        ...
                        self.gff_pos[chrom][k][0] = (
                            ...
                            + orient2
                            + "_"
                            + self.gff_pos[chrom][int(fields[3])][0]
                        )  # for locus_tag
                        self.gff_pos[chrom][k][1] = (
                            orient1
                            + "_"
                            + prev_ann[1]
                            + "<>"
                            + orient2
                            + "_"
                            + self.gff_pos[chrom][int(fields[3])][1]
                        )  # for gene name
                        self.gff_pos[chrom][k][2] = (
                            orient1
                            + "_"
                            + prev_ann[2]
                            + "<>"
                            + orient2
                            + "_"
                            + self.gff_pos[chrom][int(fields[3])][2]
                        )  # for ID
                        self.gff_pos[chrom][k][3] = (
                            orient1
                            + "_"
                            + prev_ann[3]
                            + "<>"
                            + orient2
                            + "_"
                            + self.gff_pos[chrom][int(fields[3])][3]
                        )  # for product
            ...
            for j in range(first_pos):
                ...
                self.gff_pos[chrom][j][0] = (
                    last_orient + "_" + last_ann[0] + "<>" + first_orient + "_" + first_ann[0]
                )
                self.gff_pos[chrom][j][1] = (
                    last_orient + "_" + last_ann[1] + "<>" + first_orient + "_" + first_ann[1]
                )
                self.gff_pos[chrom][j][2] = (
                    last_orient + "_" + last_ann[2] + "<>" + first_orient + "_" + first_ann[2]
                )
                self.gff_pos[chrom][j][3] = (
                    last_orient + "_" + last_ann[3] + "<>" + first_orient + "_" + first_ann[3]
                )
