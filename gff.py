...
class gff:
    def __init__(self, gff_name):
        ...
        self.gff_pos = dict()   # annotation to every position chrom/pos => [lt, name, ID]
        self.ann_pos = dict()   # positions for every annotation. Keys are ids => [ann, contig, start, stop]
            ...
            for j in range(1, len(items)):
                ...
                for k in range(int(fields[3]), int(fields[4]) + 1):
                    if self.gff_pos[chrom][k]:
                        ...
                        self.gff_pos[chrom][k] = [lt, name, id]
                ...
                if j > 1 and prev_pos < int(fields[3]):
                    ...
                    for k in range(prev_pos + 1, int(fields[3])):
                        ...
                        self.gff_pos[chrom][k][2] = orient1 + '_' + prev_ann[2] + '<>' + orient2 + \
                                                    '_' + self.gff_pos[chrom][int(fields[3])][2]  # for gene name
