...
READ_LEFT = FakeRead(
    'read_left', '3S7M', [None, None, None] + list(range(510, 517)),
    'AAA' + 'CCCCCCC', reference_name='contig_1',
)
READ_RIGHT = FakeRead(
    'read_right', '7M3S', list(range(720, 727)) + [None, None, None],
    'GGGGGGG' + 'TTT', reference_name='contig_2',
)
READ_UNMAPPED = FakeRead(
    'read_unmapped', '3S7M', [None, None, None] + list(range(510, 517)),
    'AAAAAAAAAA', is_unmapped=True, reference_name='contig_1',
)
READ_NO_CLIP = FakeRead(
    'read_no_clip', '10M', list(range(550, 560)),
    'NNNNNNNNNN', reference_name='contig_1',
)
READ_LEFT_REVERSE = FakeRead(
    'read_left_reverse', '2S5M', [None, None] + list(range(330, 335)),
    'AA' + 'CCCCC', is_reverse=True, reference_name='contig_1',
)
...
def test_parseblast_filters_by_identity_and_assigns_coordinates(tmp_path):
    ...
    blast_out_path.write_text(_blast_out_lines([
...
def test_search_forward_matches_pinned_golden_output(tmp_path):
    ...
    result = search(_fake_aln(), FORWARD_BOUNDARIES, 'ref', str(tmp_path), direction=1, run_blast=run_blast)
...
def test_search_backward_matches_pinned_golden_output(tmp_path):
    ...
    result = search(_fake_aln(), BACKWARD_BOUNDARIES, 'ref', str(tmp_path), direction=0, run_blast=run_blast)
...
def test_search_backward_no_clipped_reads_message(tmp_path):
    ...
    with pytest.raises(NoInsertionsFound, match='No clipped reads were found near estimated insertion sites.'):
